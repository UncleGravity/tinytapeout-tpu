#!/usr/bin/env bash
# Build the axi_lite_probe bitstream on the Vivado VM and pull it home.
# With --install, also scp .bit/.hwh + bench.py to the PYNQ board.

set -euxo pipefail

PROJ_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJ_NAME="axi_lite_probe"
...
Build ${PROJ_NAME}.{bit,hwh} on the Vivado VM and copy them into ./out/.
