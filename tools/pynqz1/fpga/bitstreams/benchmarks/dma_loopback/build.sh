#!/usr/bin/env bash
# Build the dma_loopback bitstream on the Vivado VM and pull it home.
# With --install, also scp .bit/.hwh + bench.py to the PYNQ board so
# Levels 1/2/3 are immediately runnable.

set -euo pipefail

PROJ_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJ_NAME="dma_loopback"
...
Build dma_loopback.{bit,hwh} on the Vivado VM and copy them into ./out/.
...
PS_PREP="New-Item -ItemType Directory -Force '${VIVADO_WORK_PARENT}' | Out-Null; Remove-Item -Recurse -Force '${VM_PROJECT}' -ErrorAction SilentlyContinue"
